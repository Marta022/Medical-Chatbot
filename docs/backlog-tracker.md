...
| 115 | 2026-04-17 | Removed translation module end-to-end from runtime, config, tests, and docs | Removed `agent/reasoning/translator.py` and all translator wiring in `ReasoningEngine`/`Orchestrator`; removed `TRANSLATION_ENABLED` from settings/env templates and `deep-translator` from dependencies; deleted translator tests and updated orchestrator/reasoning tests; synchronized architecture/README/thesis text to reflect non-translation pipeline; validated with `python -m pytest tests/test_orchestrator.py tests/test_reasoning_engine.py` (12 passed) | Optional follow-up: stabilize `tests/test_cli.py::test_eval_command_smoke` in restricted network environments |
