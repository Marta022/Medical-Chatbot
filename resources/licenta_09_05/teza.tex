...
\subsection{Interfața REST și framework-ul Flask}
...
Ollama este o platformă utilizată pentru rularea și administrarea locală a modelelor lingvistice. Aceasta oferă un API REST prin care modelele pot fi rulate, interogate și gestionate programatic, permițând integrarea lor în aplicații software fără interacțiune directă cu linia de comandă \cite{ollama_github,ollama_api}.

În cadrul proiectului realizat, Ollama este integrat ca furnizor de inferență locală. Această variantă este utilă în scenarii de testare sau prototipare în care se dorește rularea modelului pe infrastructura utilizatorului, fără dependență directă de un serviciu cloud. Modelele pot fi administrate local prin comenzi precum \texttt{ollama pull}, pentru descărcarea unui model, și \texttt{ollama serve}, pentru pornirea serverului de inferență. În implementarea proiectului, modelul activ este selectat prin variabila de mediu \texttt{OLLAMA\_MODEL}.

Ollama pune la dispoziție atât API-ul propriu, cât și o interfață compatibilă cu API-ul OpenAI, inclusiv cu formatul Chat Completions \cite{ollama_openai_compatibility}. Această compatibilitate permite conectarea aplicațiilor construite pentru formatul OpenAI la modele rulate local. În arhitectura proiectului, acest lucru permite stratului de orchestrare să comunice într-un mod uniform cu furnizori diferiți de modele, indiferent dacă răspunsul este generat printr-un serviciu cloud sau printr-un model local.

API-ul Ollama include endpoint-uri pentru generarea unui răspuns, interacțiuni de tip chat, listarea modelelor locale, descărcarea modelelor și generarea de embedding-uri \cite{ollama_api}. Astfel, Ollama poate fi utilizat atât pentru etapa de generare a răspunsurilor, cât și pentru componente auxiliare ale unui sistem RAG, în funcție de configurația aplicației.

În proiectul realizat, Ollama nu înlocuiește componentele de regăsire a contextului și nu realizează singur întregul flux RAG. Rolul său este acela de furnizor local de model, apelat de aplicație în etapa de inferență. Prelucrarea documentelor, căutarea în baza vectorială și construirea contextului rămân responsabilitatea celorlalte module ale sistemului.
...
Comparativ cu abordările clasice de tip RAG, care se bazează exclusiv pe similaritate vectorială între fragmente de text, utilizarea unui graf de cunoștințe permite exploatarea relațiilor explicite dintre concepte și valorificarea structurii globale a corpusului, ceea ce poate susține răspunsuri mai complete și mai bine contextualizate pentru întrebări care depășesc recuperarea locală a unor pasaje izolate \cite{edge2024local}.
\subsection{Graful de cunoștințe în contextul RAG}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{figuri/graph.png}
  \caption{Arhitectura GraphRAG. Sursă: \cite{edge2024local}}
  \label{fig:graph}
\end{figure}
...
și muchiile pot deține proprietăți arbitrare \cite{kuzu_doc, kuzu_cidr}. 
...
Testele sunt implementate în principal cu framework-ul standard \texttt{unittest}, iar execuția este realizată prin comenzi de tip \texttt{python -m unittest}. Suita de teste acoperă componentele critice ale sistemului. Modulul de protecție (\textit{guardrails}) este verificat pentru detectarea interogărilor periculoase, a mesajelor de urgență și a conținutului din afara domeniului medical. Modulul de orchestrare este testat pentru construcția promptului și pentru logica de reluare în caz de evaluare eșuată. Modulele de ingestie sunt verificate pentru corectitudinea extracției, segmentării și indexării documentelor.
...
\node[res, right=1.7cm of llmon] (r6a) {PERSONAL\_MEDICAL\\fără LLM};
\node[res, right=1.7cm of llm] (r6b) {PERSONAL\_MEDICAL\\confirmat LLM};
...
\draw[->] (llmon) -- node[above]{\scriptsize nu} (r6a);
\draw[->] (llmon) -- node[left]{\scriptsize da} (llm);
\draw[->] (llm) -- node[above]{\scriptsize sigur} (r6b);
\draw[->] (llm) -- (gr);
\end{tikzpicture}
...
\begin{tikzpicture}[
  box/.style={draw, rounded corners=3pt, minimum width=5cm, minimum height=0.65cm, align=center, font=\small},
  opt/.style={draw, dashed, rounded corners=3pt, minimum width=5cm, minimum height=0.65cm, align=center, font=\small},
  >=Stealth, node distance=0.55cm
]
...
\node[box, below left=0.65cm and 0.8cm of seg]   (emb)   {Generare embeddings};
\node[box, below=of emb]   (qd)    {Qdrant: indexare fragmente};
\node[opt, below right=0.65cm and 0.8cm of seg]   (ent)   {Extracție entități și relații};
\node[opt, below=of ent]   (kz)    {Kuzu: indexare graf};
...
În modul implicit, sistemul utilizează regăsirea vectorială în Qdrant, urmată de reranking pe baza scorului semantic și a suprapunerii lexicale. Traversarea grafului Kuzu este activată doar în modul \texttt{hybrid}, iar mecanismul de \textit{keyword fallback} este folosit doar atunci când rezultatele recuperate lipsesc sau au scor sub pragul minim configurat.
...
\begin{tabular}{cl}
\toprule
\textbf{Pas} & \textbf{Descriere} \\
\midrule
1 & Încarcă întrebările benchmark și cheia de răspuns \\
2 & Construiește query-uri din opțiunile A--E ale întrebării \\
3 & Recuperează context relevant din Qdrant \\
4 & Trimite întrebarea și contextul la modelul LLM \\
5 & Validează formatul răspunsului primit \\
6 & Dacă răspunsul este invalid, reîncearcă cu prompt ajustat \\
7 & Compară răspunsul cu cheia corectă \\
8 & Agregă metricile și generează raportul final \\
\bottomrule
\end{tabular}
\caption{Pașii procesului de evaluare automată pe dataset-ul benchmark medical.}
\label{tab:benchmark_steps}
\end{table}
