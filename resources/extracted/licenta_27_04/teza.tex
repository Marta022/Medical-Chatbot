...
Organizarea aplicației urmează o structură modulară, cu separare clară între
componenta de ingestie și indexare a documentelor medicale, componenta de
interogare a knowledge graph-ului gestionat prin Kuzu \cite{kuzu_doc}, pipeline-ul RAG implementat
prin modulele interne de retrieval (Qdrant + reranking + hibrid lexical), componenta
LlamaIndex utilizată punctual pentru segmentarea semantică a documentelor \cite{llamaindex_doc},
și modulul de siguranță responsabil cu detectarea și redirecționarea întrebărilor
cu caracter urgent către resurse medicale specializate.
...
Interfețele de tip REST (\textit{Representational State Transfer}) reprezintă un stil arhitectural pentru servicii web, definit prin principii precum statelessness (fiecare cerere conține toate informațiile necesare procesării sale), interfață uniformă bazată pe resurse identificate prin URI-uri și utilizarea metodelor HTTP standard (GET, POST, PUT, DELETE) cu semantică bine definită. Această abordare separă clientul de server și permite scalabilitate independentă a celor două straturi \cite{fielding2000rest}.

Flask este un micro-framework web pentru Python, proiectat pentru a oferi fundamente simple și extensibile pentru construirea aplicațiilor web și a interfețelor REST \cite{flask_doc}. Spre deosebire de framework-urile de tip \textit{full-stack} — precum Django — Flask nu impune o structură de proiect predefinită și lasă dezvoltatorului libertatea de a integra selectiv doar extensiile necesare, oferind flexibilitate în organizarea codului \cite{flask_doc}. Această filosofie de design îl face potrivit pentru aplicații cu arhitectură bine definită, în care stratul web nu reprezintă decât interfața de acces la o logică aplicativă separată.

În cadrul proiectului, Flask este utilizat pentru a expune funcționalitățile chatbot-ului ca serviciu HTTP. Această structurare separă clar interfața web de logica internă a sistemului și facilitează extensibilitatea și interoperabilitatea aplicației.
\subsection{Interfața conversațională OpenWebUI}

OpenWebUI este o aplicație web open-source pentru interacțiunea conversațională 
cu modele lingvistice, utilizată în cadrul proiectului ca strat de prezentare 
opțional peste backend-ul chatbot-ului \cite{openwebui_doc}. Spre deosebire de 
...
\texttt{/v1/models}, pe care OpenWebUI le consumă fără a necesita adaptări 
suplimentare.
...
Atunci când un sistem trebuie să găsească cele mai relevante informații 
dintr-o colecție de milioane de documente reprezentate ca vectori numerici, 
compararea interogării cu fiecare vector în parte devine prohibitiv de lentă. 
Bazele de date vectoriale rezolvă această problemă prin algoritmi de căutare 
aproximativă (\textit{Approximate Nearest Neighbor}, ANN), care identifică 
rapid rezultatele cele mai probabil relevante fără a parcurge întreaga 
colecție, acceptând un compromis minor și controlabil între viteză și 
precizie \cite{malkov2018hnsw}.
...
\textbf{Backend open-source: sentence-transformers.} Furnizorul implicit de 
...
\textbf{Backend cloud: OpenAI Embeddings.} Ca alternativă configurabilă, 
...
\texttt{SemanticSplitterNodeParser} oferit de framework adresează limitările 
...
similaritate dintre propoziții consecutive sunt calculate și comparate cu un prag 
percentil configurabil (\texttt{breakpoint\_percentile\_threshold}). Acolo unde 
...
din familia GPT prin intermediul unui API REST cu autentificare prin chei 
...
Comparativ cu abordările clasice de tip RAG, care se bazează exclusiv pe similaritate vectorială între fragmente de text, utilizarea unui graf de cunoștințe permite exploatarea relațiilor explicite dintre concepte, conducând la răspunsuri mai coerente, mai bine contextualizate și mai robuste în fața ambiguităților.
...
\texttt{csv} din Python, fără dependență runtime de tip \texttt{pandas} în fluxul
de ingestie. Fișierul CSV este parcurs secvențial cu \texttt{csv.reader}, cu
...
dezvoltatorului. În mediile containerizate, variabilele sunt injectate direct 
...
\node[sbox, right=1.8cm of grd] (blk)  {\small \texttt{OrchestratorResponse}\\small (blocat, retries=0)};
...
\begin{enumerate}
    ...
    \item \textbf{Raționamentul}: dacă interogarea este validă, controlul este transferat \texttt{ReasoningEngine} printr-un \texttt{ReasoningInput} ce conține interogarea, \texttt{top\_k}, limba și filtrele. Rezultatul \texttt{ReasoningOutput} este despachetcat direct în câmpurile \texttt{OrchestratorResponse}: răspunsul final, furnizorul și modelul utilizat, numărul de reluări, rezultatul evaluatorului, rezultatul regăsirii și liniile de context.
\end{enumerate}

Logica completă de regăsire a contextului, construcție a promptului, generare, evaluare și reluare este implementată în \texttt{ReasoningEngine} și este descrisă în detaliu în secțiunea~\ref{sec:reasoning}. Orchestratorul nu reproduce această logică — el o invocă printr-un apel unic la metoda \texttt{run}, primind înapoi un \texttt{ReasoningOutput} complet care include răspunsul, proveniența și trasabilitatea completă a procesului.

\subsection{Structura răspunsului final}

Orchestratorul returnează un obiect \texttt{OrchestratorResponse} cu câmpurile: \texttt{response} (textul răspunsului sau \texttt{None}), \texttt{provider} și \texttt{model} (furnizorul și modelul utilizat la ultima generare), \texttt{retries} (numărul de reluări efectuate), \texttt{guardrail} (\texttt{GuardrailResult} cu detaliile validării), \texttt{evaluator} (\texttt{EvaluatorResult} sau \texttt{None} în caz de respingere de guardrail), \texttt{retrieval} (\texttt{RetrievalResult} sau \texttt{None}) și \texttt{context\_lines} (lista liniilor de context utilizate). Structura serializabilă a acestui obiect permite API-ului să returneze clientului informații complete de trasabilitate privind întreg procesul de generare.
...
\node[res, right=1.5cm of v1] (r1) {UNSAFE (1.00)};
...
\node[box, below=of v4] (v5) {Informațional / Personal?};
\node[res, right=1.5cm of v5] (r5) {INFO / PERSONAL (0.90)};
\node[box, below=of v5] (llm) {Clasificator LLM (temp=0)};
...
\draw[->] (v5) -- node[left]{\scriptsize nu} (llm);
...
\node[box, below=of vec]  (lex)  {Căutare lexicală BM25 (mod hibrid)};
