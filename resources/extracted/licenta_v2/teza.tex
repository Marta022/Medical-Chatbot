...
\begin{itemize}
    \item\textbf{Similaritatea cosinus} este una dintre cele mai răspândite metrici pentru compararea reprezentărilor vectoriale. Aceasta este definită ca raportul dintre produsul scalar al celor doi vectori și produsul normelor acestora:

\begin{equation}
\text{sim}_{cos}(P, Q) = \frac{P \cdot Q}{\|P\| \|Q\|}
\end{equation}

Din punct de vedere geometric, această măsură reprezintă cosinusul unghiului dintre cei doi vectori în spațiul vectorial. Valorile apropiate de 1 indică o similaritate semantică ridicată, în timp ce valorile apropiate de 0 indică o similaritate redusă. În cazul vectorilor care nu sunt normalizați, valorile pot fi și negative, indicând o orientare opusă în spațiu. 
...
\end{equation} 
    ...
    \item\textbf{Distanța euclidiană} este una dintre cele mai cunoscute metrici de distanță în spațiul vectorial, aplicabilă în special datelor numerice. Derivată din teorema lui Pitagora, aceasta cuantifică distanța directă dintre două puncte în spațiul vectorial.

\begin{equation}
d_{L_2}(P, Q) = \sqrt{\sum_{i=1}^{d} (P_i - Q_i)^2}
\end{equation}

Valorile obținute reflectă distanța geometrică dintre doi vectori — cu cât aceasta este mai mică, cu atât similaritatea dintre reprezentări este mai ridicată.


\item\textbf{Distanța Manhattan}, cunoscută și sub denumirea de distanță L1 sau City Block Distance, cuantifică diferența dintre doi vectori prin suma diferențelor absolute ale componentelor, fără a recurge la teorema lui Pitagora.

\begin{equation}
d_{L_1}(P, Q) = \sum_{i=1}^{d} |P_i - Q_i|
\end{equation}

Geometric, metrica surprinde traseul parcurs între două puncte când deplasarea este restricționată la direcții ortogonale, analogie cu navigația pe o rețea urbană. Valorile mai mici indică o similaritate mai mare între vectori.

\item\textbf{Produsul scalar} exprimă similaritatea dintre doi vectori ca sumă a produselor componentelor corespunzătoare:

\begin{equation}
P \cdot Q = \sum_{i=1}^{d} P_i Q_i
\end{equation}

Această operație este frecvent aplicată în mecanismele de regăsire bazate pe embedding-uri datorită eficienței sale computaționale. Spre deosebire de similaritatea cosinus, rezultatul depinde și de magnitudinea vectorilor, nu doar de direcția lor. \cite{levy2024similarity}
\end{itemize}
...
Pydantic este o bibliotecă Python pentru validarea datelor și definirea modelelor prin adnotări de tip, utilizând sistemul de tipuri al Python 3 ca sursă de adevăr pentru structura datelor \cite{pydantic_doc}. Spre deosebire de clasele de date standard, modelele Pydantic validează automat tipurile și valorile câmpurilor la momentul instanțierii, generând erori clare și structurate în cazul datelor neconforme.

Pydantic este utilizat pe scară largă în ecosistemul Python pentru construirea de API-uri și servicii de date, datorită integrării sale native cu framework-uri precum FastAPI și a capacității de a serializa și deserializa automat date JSON. Validarea declarativă prin tipuri reduce volumul de cod boilerplate necesar și centralizează regulile de validare la nivelul definiției modelului, nu dispersat în logica aplicativă.

\section{Interfața API}

\subsection{Framework-ul Flask}

Interfețele de tip REST (\textit{Representational State Transfer}) reprezintă un stil arhitectural pentru servicii web, definit prin principii precum statelessness (fiecare cerere conține toate informațiile necesare procesării sale), interfață uniformă bazată pe resurse identificate prin URI-uri și utilizarea metodelor HTTP standard (GET, POST, PUT, DELETE) cu semantică bine definită. Această abordare decuplează clientul de server și permite scalabilitate independentă a celor două straturi \cite{flask_doc}.

Flask este un micro-framework web pentru Python, proiectat pentru a oferi fundamente simple și extensibile pentru construirea aplicațiilor web și a interfețelor REST \cite{flask_doc}. Spre deosebire de framework-urile de tip \textit{full-stack} — precum Django — Flask nu impune o structură de proiect predefinită și nu include componente ORM sau de autentificare implicite, oferind flexibilitate în organizarea codului și permițând integrarea selectivă a extensiilor necesare. Această filosofie de design îl face potrivit pentru aplicații cu arhitectură bine definită, în care stratul web nu reprezintă decât interfața de acces la o logică aplicativă separată.
...
Căutarea exactă a celui mai apropiat vecin (\textit{exact nearest neighbor}) într-o colecție de milioane de vectori de înaltă dimensionalitate este computațional intractabilă la scară, necesitând compararea interogării cu fiecare vector stocat. Bazele de date vectoriale rezolvă această problemă prin algoritmi de căutare aproximativă a vecinilor (\textit{Approximate Nearest Neighbor}, ANN), care reduc dramatic numărul de comparații necesare cu un cost controlabil de precizie.

Qdrant este o bază de date vectorială de înaltă performanță, proiectată pentru stocarea și regăsirea eficientă a reprezentărilor numerice de înaltă dimensionalitate \cite{qdrant_doc}. Arhitectura sa internă se bazează pe algoritmul \textit{Hierarchical Navigable Small World} (HNSW) \cite{malkov2018hnsw}, care construiește un graf navigabil pe mai multe niveluri de granularitate: nivelurile superioare conțin noduri rare și arce lungi pentru navigare rapidă, iar nivelurile inferioare conțin noduri dense și arce scurte pentru rafinarea precisă a rezultatelor. Această structură ierarhică permite căutarea cu complexitate logaritmică față de dimensiunea colecției, adecvată scenariilor de producție în care timpul de răspuns este critic.

Fiecare punct stocat în Qdrant este asociat cu un vector de embedding și un set de metadate structurate (\textit{payload}), care includ informații despre sursa documentului, pagina și secțiunea de origine — necesare pentru procesul de citare al răspunsurilor. Qdrant suportă filtrarea combinată vector+metadate, permițând restrângerea căutării la subseturi de documente pe baza câmpurilor din payload fără a sacrifica eficiența indexului vectorial.

\subsection{Generarea embedding-urilor}

Embedding-urile reprezintă proiecții ale textului în spații vectoriale de înaltă dimensionalitate, în care distanța dintre vectori reflectă gradul de similaritate semantică dintre textele corespunzătoare. Spre deosebire de reprezentările simbolice clasice — precum bag-of-words sau TF-IDF — embedding-urile dense captează relații semantice și de co-ocurență la nivel de spațiu continuu, permițând compararea conceptuală a textelor chiar și în absența termenilor comuni \cite{mikolov2013word2vec}.

Primele modele de embedding operau la nivel de cuvânt (Word2Vec, GloVe), asignând fiecărui cuvânt un vector fix, independent de context. Odată cu apariția arhitecturii Transformer și a modelelor preantrenate de tip BERT \cite{devlin2018bert}, a devenit posibilă generarea de reprezentări contextuale, în care vectorul unui cuvânt variază în funcție de propoziția din care face parte. Extinderea naturală a acestei abordări la nivel de propoziție sau paragraf a condus la modelele de tip Sentence-BERT (SBERT) \cite{reimers2019sbert}, care antrenează rețele siameze bazate pe BERT pentru a produce embedding-uri de propoziție direct comparabile prin similaritate cosinus.

\textbf{Backend open-source: sentence-transformers.} Furnizorul implicit de embedding-uri al proiectului este biblioteca \texttt{sentence-transformers} \cite{sentence_transformers_doc}, care expune o interfață Python unificată pentru o colecție extinsă de modele preantrenate. Modelul utilizat este \texttt{paraphrase-multilingual-mpnet-base-v2}, un model multilingv bazat pe arhitectura MPNet, antrenat pe date paralele din peste 50 de limbi, inclusiv română \cite{reimers2019sbert}. Modelul produce vectori de dimensiune 768, cu normalizare L2 aplicată la generare, asigurând compatibilitatea directă cu similaritatea cosinus pentru compararea fragmentelor. Avantajul principal al acestui backend este că poate rula complet local, fără a transmite date externe și fără costuri de utilizare per apel.

\textbf{Backend cloud: OpenAI Embeddings.} Ca alternativă configurabilă, proiectul integrează API-ul de embedding al OpenAI \cite{openai_embeddings}, suportând modelele \texttt{text-embedding-3-small} (1536 dimensiuni), \texttt{text-embedding-3-large} (3072 dimensiuni) și \texttt{text-embedding-ada-002} (1536 dimensiuni). Modelele din seria \texttt{text-embedding-3} au fost antrenate cu o funcție de pierdere de tip Matryoshka Representation Learning, care permite reducerea dimensionalității fără pierderi semnificative de calitate. Alegerea între cele două backend-uri se face prin variabila de mediu \texttt{EMBEDDING\_PROVIDER}, fără a necesita modificări ale logicii de regăsire.
...
LlamaIndex (cunoscut anterior ca GPT Index) este un framework Python specializat pentru construirea aplicațiilor bazate pe LLM care necesită integrarea datelor externe \cite{llamaindex_doc}. Framework-ul oferă abstracții de nivel înalt pentru ingestia, transformarea, indexarea și interogarea documentelor, acoperind întregul ciclu de viață al unui pipeline RAG: încărcare din surse heterogene, segmentare, generare de embedding-uri, stocare în baze de date vectoriale și construcție de interogări compuse.

Strategia de segmentare reprezintă una dintre deciziile cu cel mai mare impact asupra calității regăsirii. Abordările tradiționale — segmentare la număr fix de tokeni sau la delimitatori structurali — pot produce fragmente care taie unități semantice coerente sau agregă paragrafe tematice distincte. Modulul de segmentare semantică al LlamaIndex (\textit{SemanticSplitterNodeParser}) adresează această problemă prin calculul similarității embedding-urilor propozițiilor consecutive și introducerea de granițe de fragment acolo unde similaritatea scade sub un prag adaptiv. Rezultatul este o segmentare care respectă coerența tematică a textului, producând fragmente mai adecvate pentru construcția contextului în aplicații RAG \cite{llamaindex_doc}.

\section{Modele lingvistice și rutare multi-provider}

\subsection{Furnizorii de servicii cloud: OpenAI și Anthropic}

OpenAI reprezintă furnizorul principal al sistemului, oferind acces la modele din familia GPT prin intermediul unui API REST cu autentificare prin chei \cite{openai_api}. Seria GPT-4 — în special modelele \texttt{gpt-4o} și \texttt{gpt-4o-mini} — combină capacitate de raționament complex cu timp de răspuns redus și costuri operaționale moderate. Modelele GPT sunt caracterizate printr-o capacitate ridicată de urmărire a instrucțiunilor complexe, generare de text coerent în limba română și suport pentru prompturi structurate pe roluri (system, user, assistant), ceea ce le face potrivite pentru domeniul medical informațional unde precizia și tonul răspunsului sunt esențiale \cite{openai_api}.

Anthropic Claude reprezintă al doilea furnizor cloud integrat \cite{anthropic_claude}. Modelele din seria Claude 3 — Haiku, Sonnet și Opus — sunt antrenate cu o metodologie de tip Constitutional AI, care introduce un set explicit de principii etice în procesul de antrenare prin reinforcement learning din feedback AI (RLAIF). Aceasta le conferă un comportament mai conservator și mai consistent în situații ambigue sau cu potențial de risc, caracteristici deosebit de relevante pentru un chatbot medical. Integrarea se realizează prin SDK-ul oficial Python (\texttt{anthropic}), care expune o interfață similară cu cea a OpenAI, simplificând implementarea stratului de abstractizare comun.

\subsection{Inferența locală cu Ollama}

Pentru scenariile care necesită funcționare fără conexiune la internet sau care nu permit transmiterea datelor către servicii externe din considerente de confidențialitate, proiectul integrează Ollama ca furnizor de inferență locală \cite{ollama_doc}. Rularea locală a modelelor lingvistice de mari dimensiuni este posibilă prin tehnici de cuantizare — reprezentarea parametrilor în formate de precizie redusă (INT4, INT8) față de float16 sau float32 standard — care reduc semnificativ cerințele de memorie și putere de calcul cu pierderi minime de calitate. Formatele precum GGUF (utilizat de llama.cpp) permit distribuirea modelelor cuantizate și rularea lor eficientă inclusiv pe hardware de consum.

Ollama abstractizează gestionarea modelelor locale — descărcare, versionare, rulare — și expune o interfață REST compatibilă cu formatul OpenAI Chat Completions. Proiectul utilizează modele din familiile LLaMA, Gemma și Qwen, disponibile în variante cuantizate de diferite dimensiuni. Compatibilitatea de protocol cu OpenAI elimină necesitatea unor adaptoare separate pe furnizor și simplifică adăugarea de noi modele locale în configurație fără a modifica logica de orchestrare.
...
Kuzu implementează modelul de date Property Graph, în care atât nodurile cât și muchiile pot deține proprietăți arbitrare, și suportă limbajul de interogare Cypher — standard de facto în ecosistemul grafurilor de proprietăți, familiar din Neo4j. Optimizările interne ale Kuzu vizează interogări analitice de tip OLAP pe grafuri, cu suport pentru join-uri eficiente pe muchii și traversal multi-hop, spre deosebire de sistemele OLTP orientate pe actualizări frecvente \cite{kuzu_doc}.
...
pandas este o bibliotecă Python de referință pentru analiza și manipularea datelor structurate, construită pe NumPy și optimizată pentru operații vectorizate pe seturi de date tabelare \cite{pandas_doc}. Structura centrală a bibliotecii — \texttt{DataFrame} — oferă o reprezentare bidimensională etichetată, cu suport nativ pentru indexare, filtrare, agregare, join-uri și transformări pe coloane, operații care în absența sa ar necesita bucle explicite sau instrumente specializate.

pandas suportă citirea și scrierea unui spectru larg de formate de date: CSV, JSON, Excel, Parquet, SQL — printr-o interfață uniformă — ceea ce simplifică integrarea surselor heterogene fără adaptoare specifice per format. Operațiile de preprocesare a textului, normalizare și filtrare pot fi exprimate declarativ prin operații pe serii, menținând codul concis și ușor de auditat \cite{pandas_doc}.

\subsection{Gestionarea configurației și apelurile HTTP auxiliare}

Principiul \textit{twelve-factor app} recomandă stocarea configurației exclusiv în variabile de mediu, separând-o complet de codul sursă pentru a permite deployment-ul aceleiași imagini în medii diferite (dezvoltare, testare, producție) fără modificări ale codului. Biblioteca \texttt{python-dotenv} implementează acest principiu prin încărcarea variabilelor dintr-un fișier \texttt{.env} în mediul de execuție al procesului, cu suport pentru suprascrierea valorilor din fișier cu variabile definite direct în sistem — comportamentul așteptat în mediile containerizate.

\texttt{httpx} este un client HTTP modern pentru Python, compatibil cu API-ul bibliotecii standard \texttt{requests}, dar cu suport nativ pentru HTTP/2, conexiuni asincrone (\texttt{async}/\texttt{await}) și timeout-uri configurabile pe mai multe niveluri. Este utilizat în proiect atât pentru apeluri HTTP auxiliare în fluxurile de date, cât și pentru testarea programatică a endpoint-urilor REST ale aplicației, unde capacitatea de a efectua cereri sincrone și asincrone din același client simplifică suita de teste.

\section{Containerizare și rulare}

\subsection{Principiile containerizării cu Docker}

Docker reprezintă una dintre cele mai utilizate platforme pentru containerizarea aplicațiilor software, oferind un mecanism de ambalare a aplicației împreună cu dependențele și configurațiile sale într-o imagine portabilă și reproductibilă \cite{docker_doc}. Containerele Docker folosesc mecanisme ale nucleului Linux, precum namespace-urile și cgroup-urile, pentru a oferi izolare la nivel de proces și control asupra resurselor. În practică, această abordare asigură un mediu de execuție mult mai consistent între dezvoltare, testare și producție și reduce problemele clasice de incompatibilitate a dependențelor, fără a implica însă izolarea completă specifică unei mașini virtuale.
...
Sistemul implementat este organizat ca o arhitectură modulară de tip agentic, în care fiecare componentă are o responsabilitate bine delimitată și poate fi testată, înlocuită sau extinsă independent. Organizarea internă respectă principiul separării responsabilităților (\textit{separation of concerns}): modulul \texttt{agent} conține orchestratorul principal, mecanismele de protecție, evaluatorul și modulul de raționament; modulul \texttt{rag} gestionează segmentarea documentelor, generarea embedding-urilor și interogarea bazei vectoriale; modulul \texttt{knowledge} cuprinde extractorul de entități și componenta de graf de cunoștințe; modulul \texttt{api} expune interfața REST, iar \texttt{config} centralizează configurația și definițiile de prompturi. Contractele de date dintre module sunt definite explicit în pachetul \texttt{models} prin modele Pydantic, garantând consistența tipurilor pe întregul flux de execuție.

Fluxul de procesare al unei interogări urmează o succesiune secvențială de etape, proiectată pentru a reduce riscul de răspunsuri incorecte în context medical:
...
Pe lângă regăsirea vectorială pură, sistemul suportă un mod de regăsire hibrid, care combină similaritatea vectorială cu o componentă de căutare lexicală. Această combinație permite identificarea documentelor relevante atât prin similitudine semantică, cât și prin potrivire exactă a termenilor tehnici — deosebit de importantă în medicină, unde termenii de specialitate nu au sinonime comune. Peste rezultatele inițiale se poate aplica un pas de \textit{reranking}, iar în situațiile de scor vectorial redus este disponibil un mecanism de \textit{keyword fallback}.
