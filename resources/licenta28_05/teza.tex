...
Arhitectura modelelor lingvistice de mari dimensiuni presupune preluarea datelor text din multiple surse, urmată de transmiterea acestora către etapele de preprocesare. Procesul de antrenare include mai multe etape, precum inițializarea aleatorie a parametrilor, transformarea datelor în format numeric, calculul funcției de pierdere, optimizarea parametrilor și antrenarea iterativă a modelului.
...
După cum se observă în Figura~\ref{fig:LLM_stages}, antrenarea LLM se poate împărți în mai mulți pași. După filtrarea datelor, 
...
\section{Persistența operațională cu SQLite}

SQLite este un motor de baze de date SQL embedded, fără proces server separat, care păstrează o bază de date completă într-un singur fișier local \cite{sqlite_doc}. Această caracteristică este adecvată pentru jurnalizarea operațională din proiect, deoarece aplicația poate scrie tranzacțional date despre conversații prin modulul standard Python \texttt{sqlite3}, fără introducerea unui serviciu suplimentar în infrastructura de rulare.

Rolul SQLite este distinct de rolurile bazelor Qdrant și Kuzu. Qdrant păstrează reprezentările vectoriale și conținutul fragmentelor utilizabile pentru regăsire semantică, iar Kuzu susține modelarea relațiilor dintre entități. SQLite nu participă la selectarea contextului și nu funcționează ca memorie conversațională; el păstrează o evidență structurată a interacțiunilor procesate și a identificatorilor fragmentelor care au contribuit la răspunsul final. În implementarea curentă, utilizarea sa este configurabilă prin variabilele \texttt{INTERACTION\_DB\_ENABLED} și \texttt{INTERACTION\_DB\_PATH}, calea implicită fiind \texttt{data/app/interactions.db}.
...
Testele sunt implementate în principal cu framework-ul standard \texttt{unittest}, iar execuția este realizată prin comenzi de tip \texttt{python -m unittest}. Suita de teste acoperă componentele critice ale sistemului. Modulul de protecție (\texttt{guardrails}) este verificat pentru detectarea interogărilor periculoase, a mesajelor de urgență și a conținutului din afara domeniului medical. Modulul de orchestrare este testat pentru construcția promptului și pentru logica de reluare în caz de evaluare eșuată. Modulele de regasire a informației sunt verificate pentru corectitudinea extracției, segmentării și indexării documentelor.
...
Evaluarea experimentală urmărește să măsoare în ce măsură sistemul poate utiliza informațiile medicale indexate pentru a rezolva întrebări de tip grilă. În acest context, prin „învățarea informațiilor” nu se înțelege reantrenarea parametrilor modelelor lingvistice, ci capacitatea agentului RAG de a regăsi, integra și aplica corect fragmentele relevante din corpusul medical disponibil. Astfel, testarea vizează întregul flux de inferență: recuperarea contextului, constrângerea răspunsului la informațiile recuperate, respectarea formatului cerut și compararea răspunsului final cu cheia de evaluare.

Modulul de benchmark --- implementat în \texttt{agent/benchmarking/benchmark.py} --- este distinct față de modulul de evaluare prezentat în secțiunea~\ref{sec:evaluare}. Modulul de evaluare face parte din pipeline-ul de răspuns al sistemului și validează calitatea unui singur răspuns generat în timp real. Modulul de benchmark operează în afara acestui flux: rulează un set standardizat de întrebări pe mai multe modele LLM, în condiții identice, cu scopul de a produce metrici comparabile între configurații diferite.

Pentru fiecare întrebare, agentul construiește promptul de testare, apelează componenta RAG pentru recuperarea contextului, transmite promptul către modelul LLM selectat, parsează răspunsul și aplică o validare structurală și semantică. Rezultatul este apoi comparat cu răspunsul corect din cheia de evaluare. Această arhitectură permite separarea erorilor de retrieval, a erorilor de raționament și a erorilor de format, ceea ce oferă o imagine mai precisă asupra comportamentului sistemului decât simpla verificare manuală a răspunsurilor.
...
Răspunsurile care nu trec validarea sunt reluate cu un mesaj de revizuire. Pentru itemii cu răspuns unic există și un repair prompt suplimentar, care impune explicit alegerea unei singure litere atunci când modelul returnează mai multe.
...
În colecția Qdrant utilizată de aplicație sunt indexate date structurate din fișierele \texttt{disease\_database.json} și \texttt{dataset\_sheet1.csv}, precum și fragmente textuale provenite din cele trei capitole medicale \texttt{cap1\_revizuit.md}, \texttt{cap2\_revizuit.md} și \texttt{cap3\_revizuit.md}.
...
\hline
\texttt{disease\_database.json} & JSON, 47 afecțiuni în 4 categorii & Date structurate despre afecțiuni, simptome, tratament, complicații și prevenție, provenite dintr-un dataset disponibil pe platforma Kaggle. \\
\hline
\texttt{dataset\_sheet1.csv} & CSV, 153 înregistrări & Date tabelare despre afecțiuni, simptome și recomandări generale, provenite dintr-un dataset disponibil pe platforma Kaggle. \\
\hline
...
Cele trei capitole Markdown provin din manualul scanat \textit{Semiologie medicală. Note de curs -- elemente de semiologie generală, respiratorie, cardiovasculară, digestivă, renourinară}, de Dr. Dorin Dragoş, publicat la Editura Universitară „Carol Davila”, Bucureşti, în anul 2013. Titlul, autorul, editura și anul sunt vizibile pe prima pagină a fișierului sursă \texttt{DORIN-CURS\_SEM2\_searchable.pdf}. Fragmente din toate cele trei capitole se regăsesc în colecția Qdrant utilizată în aplicație.

Fișierele structurate \texttt{disease\_database.json} și \texttt{dataset\_sheet1.csv} au fost preluate din platforma Kaggle și completează corpusul de curs cu informații sintetice despre afecțiuni și simptome. Astfel, baza vectorială reunește date medicale structurate provenite din Kaggle și fragmente de semiologie provenite din suportul universitar de curs.

Setul de testare conține 100 de întrebări medicale de tip grilă, fiecare având asociată o cheie de răspuns completă. Întrebările includ atât itemi cu răspuns unic, cât și itemi cu răspuns multiplu, secvențe, asocieri și construcții de frază. Această diversitate este relevantă pentru evaluarea sistemului, deoarece obligă agentul să trateze nu doar regăsirea factuală, ci și aplicarea informației în formate de examinare diferite.
...
\subsection{Experimentul 1: Evaluarea comparativă a modelelor LLM}
