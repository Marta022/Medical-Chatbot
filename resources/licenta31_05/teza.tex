...
Vectorizarea prin OpenAI este opțională, iar vectorizarea locală rămâne configurația implicită.
...
În implementare, fragmentele recuperate din Qdrant și relațiile extrase din Kuzu sunt reunite în același rezultat de retrieval, transmis ulterior motorului de raționament.
...
O verificare sintetică poate evidenția câte fragmente au fost asociate fiecărei solicitări, fiind utilă pentru depanarea configurației de retrieval și pentru validarea jurnalizării.

\begin{center}
\begin{minipage}{0.68\textwidth}
\begin{lstlisting}[language=SQL, caption={Numărul fragmentelor recuperate pentru fiecare interacțiune}]
SELECT
    i.created_at,
    i.user_query,
    COUNT(r.rank) AS retrieved_chunks
FROM interactions AS i
LEFT JOIN retrieved_chunk_refs AS r
    ON r.interaction_id = i.id
GROUP BY i.id, i.created_at, i.user_query
...
\section{Evaluarea învățării informațiilor medicale}

În această etapă a fost evaluată capacitatea sistemului de a răspunde la întrebări medicale de tip grilă pe baza informațiilor disponibile în corpusul indexat. Scopul testării a fost de a observa cât de bine poate agentul să identifice informațiile relevante, să le folosească în formularea răspunsului și să aleagă varianta corectă dintre opțiunile disponibile.

Evaluarea a fost realizată printr-un modul de benchmark, implementat în \texttt{agent/benchmarking/benchmark.py}. Acesta rulează automat un set de întrebări prestabilite și compară răspunsurile generate de model cu soluțiile corecte din cheia de evaluare. Spre deosebire de verificarea realizată în timpul unei conversații obișnuite cu utilizatorul, benchmark-ul permite testarea sistemului în mod repetabil, pe aceleași întrebări și în aceleași condiții.

Pentru fiecare întrebare, sistemul pornește de la enunțul grilei și de la variantele de răspuns asociate. Apoi, componenta RAG caută în baza de cunoștințe fragmentele medicale relevante, care sunt transmise mai departe către modelul lingvistic. Modelul generează răspunsul final, iar acesta este prelucrat automat pentru a putea fi comparat cu răspunsul corect.

Prin această metodă nu se urmărește doar numărul de răspunsuri corecte, ci și modul în care sistemul ajunge la acestea. O eroare poate apărea în mai multe puncte ale fluxului: contextul recuperat poate fi insuficient, modelul poate interpreta greșit informația primită sau răspunsul poate fi generat într-un format care nu poate fi validat automat. Din acest motiv, benchmark-ul oferă o imagine mai clară asupra performanței sistemului decât o simplă analiză manuală a câtorva răspunsuri individuale.

Rezultatele obținute permit compararea mai multor modele LLM integrate în aceeași arhitectură RAG. Deoarece setul de întrebări și mecanismul de evaluare rămân aceleași, diferențele dintre scoruri reflectă mai bine capacitatea fiecărui model de a utiliza contextul recuperat și de a selecta răspunsul corect.


\subsection{Detectarea tipului de grilă și generarea prompturilor}

În cadrul benchmark-ului, întrebările medicale nu au toate aceeași structură. Unele solicită alegerea unei singure variante corecte, altele presupun selectarea mai multor răspunsuri, ordonarea unor etape sau realizarea unor asocieri între elemente. Din acest motiv, folosirea unui singur prompt pentru toate întrebările ar putea duce la răspunsuri ambigue sau greu de evaluat automat.

Pentru a evita această problemă, agentul identifică mai întâi tipul grilei. Clasificarea se face pe baza unor indicii din formularea întrebării și din modul în care sunt construite variantele de răspuns. De exemplu, anumite expresii pot indica faptul că întrebarea cere mai multe răspunsuri corecte, în timp ce alte structuri sugerează o alegere unică, o ordine cronologică sau o asociere între două coloane.

După identificarea tipului de grilă, sistemul generează un prompt adaptat acelei categorii. Fiecare prompt impune un format clar al răspunsului, astfel încât rezultatul produs de model să poată fi extras și comparat automat cu cheia de evaluare.

Fiecare prompt impune câmpuri de ieșire obligatorii adaptate tipului de grilă. Pentru întrebările de tip \texttt{R.I.} și \texttt{u.a.s.c.c.e.}, câmpurile obligatorii sunt \texttt{STATUS\_A} până la \texttt{STATUS\_E} (cu valorile \texttt{ADEVARAT} sau \texttt{FALS}) și \texttt{ANSWER} (literele finale selectate):
...
La tipul \texttt{u.a.s.c.c.e.}, răspunsul final este format din literele marcate cu \texttt{FALS}. La tipul \texttt{C.d.d.} (alegere unică), structura este identică, dar \texttt{ANSWER} conține o singură literă:
...
Pentru întrebările de tip \texttt{F.d.u.} / \texttt{F.d.d.} (secvență temporală), modelul reconstruiește ordinea din context și o mapează la una dintre variantele A--E:
...
Pentru întrebările de tip \texttt{s.c.f.c.e.} (construire frază), modelul generează fraza completă în câmpul \texttt{FRAZA}, identifică fragmentul problematic în \texttt{ISSUE\_FRAGMENT}, iar \texttt{ANSWER} conține litera corespunzătoare:
...
\subsection{Strategia de regăsire a informațiilor în sistemul RAG}

Pentru etapa de benchmark, integrarea cu componenta RAG nu s-a bazat doar pe o interogare simplă construită din textul întrebării. În cazul grilelor medicale, această abordare poate fi insuficientă, deoarece informația necesară pentru alegerea răspunsului corect nu se află întotdeauna într-un singur fragment de text. De multe ori, fiecare variantă de răspuns face referire la un concept diferit, la un simptom, la o situație clinică sau la o relație particulară dintre mai multe noțiuni.

Din acest motiv, pentru fiecare întrebare a fost construit un set extins de interogări. Pe lângă întrebarea inițială, sistemul generează câte o interogare separată pentru fiecare variantă de răspuns, prin combinarea enunțului cu textul opțiunii respective. Astfel, componenta de regasire a informației are șanse mai mari să recupereze fragmente relevante nu doar pentru întrebarea generală, ci și pentru fiecare posibil răspuns analizat.

Rezultatele obținute din aceste interogări sunt apoi reunite și reordonate printr-un mecanism de reordonare. Scorul final al fiecărui fragment ține cont de mai mulți factori: scorul semantic returnat inițial de sistemul de căutare, gradul de suprapunere lexicală cu textul întrebării și potrivirea cu termenii specifici fiecărei variante de răspuns. În acest fel, fragmentele care sunt relevante atât semantic, cât și lexical pentru întrebarea evaluată sunt prioritizate.

După etapa de reordonare, se elimină fragmentele duplicate, pentru a evita repetarea aceluiași conținut în contextul transmis modelului. La final, cele mai relevante top-\textit{k} fragmente sunt concatenate și folosite ca suport informațional pentru modelul de inferență.

Această strategie îmbunătățește calitatea contextului oferit modelului, mai ales în cazul întrebărilor cu mai multe variante apropiate semantic. În loc ca modelul să primească doar fragmente legate de formularea generală a întrebării, acesta primește informații selectate și în funcție de conținutul fiecărei opțiuni, ceea ce reduce riscul ca răspunsul final să fie ales pe baza unui context incomplet.

\subsection{Inferența și validarea răspunsurilor}

În etapa de inferență, agentul folosește un \textit{system prompt} formulat astfel încât modelul să se bazeze cât mai strict pe fragmentele recuperate din corpus. Modelului i se cere să răspundă doar pe baza contextului primit, iar folosirea cunoștințelor externe este limitată în mod explicit. Această constrângere este importantă deoarece scopul evaluării nu este verificarea memoriei interne a modelului lingvistic, ci analiza modului în care sistemul RAG reușește să găsească și să folosească informația relevantă.


Răspunsul generat este verificat în două etape. Prima etapă urmărește structura răspunsului și verifică dacă sunt prezente câmpurile cerute pentru tipul respectiv de întrebare. De asemenea, se verifică dacă există concordanță între răspunsul final și câmpurile intermediare. De exemplu, pentru întrebările cu răspuns unic, se verifică potrivirea dintre câmpul \texttt{ANSWER} și valorile asociate variantelor individuale. Pentru întrebările de ordonare, asociere sau completare de fragment, se verifică dacă răspunsul final corespunde câmpurilor dedicate, precum \texttt{ORDER}, \texttt{ASSOCIATIONS} sau \texttt{ISSUE\_FRAGMENT}.


A doua etapă urmărește corectitudinea logică a răspunsului. În această fază se verifică dacă literele finale au fost obținute corect din valorile intermediare și dacă explicația implicită a modelului este susținută de contextul recuperat. Răspunsurile care introduc presupuneri, formulări speculative sau raționamente care nu pot fi legate de fragmentele oferite sunt considerate nevalide.

Dacă un răspuns nu trece aceste verificări, sistemul poate relua generarea printr-un mesaj de revizuire, în care modelului i se indică problema observată și i se cere să producă un răspuns corectat. Pentru întrebările cu răspuns unic, există și un \textit{repair prompt} suplimentar, folosit atunci când modelul returnează mai multe variante deși formatul impune alegerea unei singure litere.

\subsection{Raportarea rezultatelor}

Rezultatul final al modulului este un raport organizat pe două niveluri de detaliu. Pentru fiecare întrebare sunt salvate răspunsul brut generat de model, literele prezise, literele corecte din cheia de evaluare, eventualele motive de respingere și fragmentele recuperate prin componenta RAG. Aceste informații permit analizarea fiecărui răspuns în parte și ajută la identificarea cauzei unei erori, fie că aceasta apare în etapa de recuperare a contextului, în respectarea formatului cerut sau în interpretarea informației medicale.

Prin această organizare, modulul nu are doar rolul de a nota automat răspunsurile, ci oferă și o imagine mai clară asupra comportamentului sistemului LLM-RAG. Rezultatele obținute pot fi folosite pentru a observa unde apar cele mai frecvente probleme și pentru a separa erorile produse în etapa de recuperare a contextului de cele apărute în etapa de interpretare sau de formatare a răspunsului.



\section{Setul de date folosit}

În colecția Qdrant utilizată de aplicație sunt indexate fragmente textuale provenite din cele patru capitole medicale \texttt{cap1\_revizuit.md}, \texttt{cap2\_revizuit.md}, \texttt{cap3\_revizuit.md} și \texttt{cap4\_revizuit.md}.

\begin{table}[H]
\centering
\caption{Sursele de date indexate în Qdrant}
\label{tab:surse-ingestie}
\small
\begin{tabular}{|p{4.2cm}|p{2.3cm}|p{6.5cm}|}
\hline
\textbf{Fișier} & \textbf{Tip / dimensiune} & \textbf{Conținut și rol în sistem} \\
\hline
\texttt{cap1\_revizuit.md} & Markdown & Capitolul \textit{Semiologie generală}, centrat pe febră, hipertermie și evaluarea acestora. \\
\hline
\texttt{cap2\_revizuit.md} & Markdown & Capitolul \textit{Semiologie ORL şi oftalmologică}, incluzând structuri și semne clinice oculare și ORL. \\
\hline
\texttt{cap3\_revizuit.md} & Markdown & Capitolul \textit{Semiologia țesutului subcutanat}, incluzând hidratarea, deshidratarea și edemul. \\
\hline
\texttt{cap4\_revizuit.md} & Markdown & Capitol medical revizuit și transformat într-un format textual utilizabil pentru indexare și regăsire semantică. \\
\hline
\end{tabular}
\end{table}

Cele patru fișiere Markdown au fost obținute prin prelucrarea capitolelor din manualul \textit{Semiologie medicală. Note de curs -- elemente de semiologie generală, respiratorie, cardiovasculară, digestivă, renourinară}, scris de Dr. Dorin Dragoş și publicat la Editura Universitară „Carol Davila”, Bucureşti, în anul 2013. Capitolele au fost revizuite și transformate într-un format textual mai ușor de indexat, pentru a putea fi folosite de componenta RAG în etapa de recuperare a contextului.

Setul de testare conține 100 de întrebări medicale de tip grilă, fiecare având asociată o cheie de răspuns completă. Întrebările includ atât cazuri cu răspuns unic, cât și cazuri cu răspuns multiplu, secvențe, asocieri și construcții de frază. Această diversitate este relevantă pentru evaluarea sistemului, deoarece obligă agentul să trateze nu doar regăsirea factuală, ci și aplicarea informației în formate de examinare diferite.

\section{Metrici de evaluare}

Evaluarea performanței unui model pe setul de întrebări de tip grilă nu poate fi redusă la o singură cifră, deoarece întrebările medicale admit atât răspunsuri unice, cât și răspunsuri multiple. Din această cauză, modulul calculează un set complet de metrici, grupate în trei categorii: metrici pe întrebare individuală, metrici \textit{macro} (agregate prin mediere pe întrebări) și metrici \textit{micro} (agregate prin cumularea erorilor la nivel global).
...
\subsection{Precision, Recall și F1 pe întrebare}

Pe baza valorilor TP, FP și FN, se calculează pentru fiecare întrebare în parte:
...
\begin{itemize}
    ...
    F1 este metrica principală la nivel de întrebare, deoarece recompensează simultan acuratețea și completitudinea răspunsului.
\end{itemize}

Toate trei sunt definite ca $0$ când numitorul este $0$ (model fără nicio predicție sau întrebare fără niciun răspuns corect în cheie).
...
Această metrică este mai strictă decât F1: o întrebare cu răspuns $\{A, C\}$ pentru care modelul prezice $\{A, B, C\}$ obține $F1 = 0.8$, dar contribuie cu $0$ la exact match rate. Exact match rate este relevantă în special pentru întrebările cu răspuns unic, unde orice abatere este o eroare completă, și constituie un indicator direct al ratei de răspuns corect în sens clasic de grilă.
...
Pentru fiecare dintre aceste interogări este recuperat un număr extins de fragmente candidate. Rezultatele sunt reunite într-un singur set, duplicatele sunt eliminate, iar fragmentele sunt reordonate folosind un scor care combină similaritatea semantică, suprapunerea lexicală cu întrebarea și potrivirea cu textele variantelor. După această etapă sunt păstrate numai primele trei fragmente cu semnalul cel mai relevant, care formează contextul transmis modelului.
...
Al doilea experiment urmărește efectul constrângerii formatului de răspuns asupra performanței agentului de benchmark. Comparația a fost realizată între rulările \texttt{output\_v5} și \texttt{output\_v6}, ambele executate pe același set inițial de 10 întrebări din \texttt{primele\_10\_grile\_pag2\_curatate.json}, cu aceeași cheie de răspuns și cu guardrail-ul dezactivat. Diferența principală dintre cele două rulări nu este setul de date, ci modul în care modelul este instruit să formuleze răspunsul.
...
Experimentul arată astfel că, după identificarea problemei de retrieval din rularea inițială, o altă sursă importantă a scorurilor mici era formatul prea liber al răspunsurilor. Prima ameliorare importantă asupra generării a venit din constrângerea formatului, iar îmbunătățirile ulterioare au vizat și separarea explicită a fragmentelor recuperate, reordonarea rezultatelor și extinderea benchmark-ului.

\subsection{Experimentul 3: Introducerea prompturilor adaptive pe tipuri de grilă}

Al treilea experiment compară rulările \texttt{output\_v14} și \texttt{output\_v16\_4o}, executate pe același set de 21 de întrebări, cu aceeași cheie de răspuns și cu guardrail-ul dezactivat. Modificarea analizată este introducerea prompturilor adaptive, construite în funcție de tipul grilei. În locul unei instrucțiuni generale pentru toate întrebările, noul flux precizează separat regula de interpretare și formatul de răspuns necesar pentru fiecare categorie de grilă.
...
\texttt{output\_v16\_4o} & 21 & 0,8254 & 0,7619 & 0,8254 \\
\bottomrule
\end{tabular}
\end{table}
...
În \texttt{output\_v16\_4o}, promptul adaptat explică explicit ce litere trebuie incluse în răspuns pentru acest tip de întrebare. Pentru aceeași întrebare, modelul păstrează aceleași statusuri intermediare, dar aplică corect regula și produce răspunsul:
...
Prin urmare, saltul de la 0,6190 la 0,8254 este asociat introducerii prompturilor adaptive pe tipuri de grilă. Rata exact match crește de la 61,90\% la 76,19\%, iar erorile de interpretare a cerinței finale sunt reduse în mai multe întrebări. De exemplu, întrebarea 521 trece de la \texttt{B,E} la \texttt{A,C,D}, întrebarea 534 trece de la \texttt{A,B,C,E} la \texttt{D}, iar întrebarea 536 trece de la \texttt{A,C,D,E} la \texttt{B}. Deoarece între cele două rulări diferă și modelul LLM utilizat, variația scorului nu poate fi atribuită exclusiv prompturilor; exemplele individuale demonstrează însă corectarea regulii de interpretare urmărite de acest experiment.

\subsection{Experimentul 4: Introducerea reluărilor pentru răspunsuri invalide}

Al patrulea experiment a vizat robustețea benchmark-ului în situațiile în care modelul nu returnează un răspuns utilizabil. Comparația relevantă este între rulările \texttt{output\_v16\_4o} și \texttt{output\_v17\_4o}, ambele executate pe același set de 21 de întrebări. În \texttt{output\_v16\_4o} apar încă răspunsuri brute goale, ceea ce duce automat la predicții vide și scor 0 pentru întrebările respective. În \texttt{output\_v17\_4o}, răspunsurile goale dispar, iar cazurile problematice sunt fie recuperate, fie marcate explicit prin câmpul \texttt{rejection\_reason}.
...
Efectul se observă direct pe întrebările afectate. În \texttt{output\_v16\_4o}, întrebările 526 și 533 au \texttt{raw\_response} gol și nu produc nicio predicție. În \texttt{output\_v17\_4o}, întrebarea 533 este recuperată și ajunge la răspunsul corect \texttt{C}, cu \texttt{exact\_match=true}. Întrebarea 526 nu mai are răspuns gol, dar rămâne invalidă structural, fiind marcată cu \texttt{rejection\_reason=missing\_answer\_letters}. Astfel, benchmark-ul nu mai pierde tăcut aceste cazuri: fie obține un răspuns valid la reluare, fie păstrează explicit motivul eșecului.
...
După obținerea unei variante stabile a benchmark-ului, evaluarea a fost extinsă la 100 de întrebări medicale de tip grilă și la mai multe modele LLM. În această etapă nu s-a mai urmărit efectul unei singure modificări interne, ci comportamentul modelelor în aceleași condiții experimentale. Rezultatele au fost extrase din rulările \texttt{output\_v33}--\texttt{output\_v37}, cu excluderea variantei locale Gemma din tabelul principal, deoarece performanța acesteia a fost mult sub nivelul modelelor cloud evaluate.
...
Un rezultat important al lucrării este faptul că performanța finală nu depinde doar de modelul LLM ales. Strategia de retrieval, modul de construcție al promptului, validarea structurală a răspunsului și mecanismele de retry influențează direct calitatea sistemului. Ceea ce a funcționat bine a fost separarea clară a responsabilităților între module și introducerea unui evaluator care poate respinge răspunsurile incomplete, speculative sau greu de parsat. De asemenea, folosirea unor prompturi diferite în funcție de tipul întrebării de tip grilă a ajutat la reducerea ambiguității și la obținerea unor răspunsuri mai ușor de evaluat automat.
...
În concluzie, lucrarea demonstrează că o arhitectură agentică bazată pe LLM și RAG poate fi folosită ca suport pentru învățare, documentare și verificare în domeniul medical, cu condiția ca răspunsurile să fie ancorate în surse indexate și validate printr-un flux automatizat. Sistemul realizat nu înlocuiește expertiza medicală și nu trebuie interpretat ca instrument de diagnostic, însă oferă o bază funcțională pentru aplicații educaționale și de asistență informațională. Rezultatele obținute susțin utilitatea abordării propuse și arată că direcția cea mai promițătoare este combinarea modelelor lingvistice performante cu retrieval robust, validare strictă și mecanisme transparente de raportare a limitărilor.

\section{Direcții viitoare}

O primă direcție de dezvoltare constă în extinderea bazei vectoriale prin integrarea unui număr mai mare de documente medicale provenite din surse validate și actualizate periodic. Creșterea diversității corpusului ar permite sistemului să răspundă la întrebări din mai multe specialități și ar reduce situațiile în care contextul recuperat este insuficient. Procesul de extindere ar trebui însoțit de verificarea surselor, eliminarea informațiilor duplicate și monitorizarea calității fragmentelor rezultate în urma segmentării.

O altă direcție importantă este dezvoltarea unei interfețe web accesibile de la distanță și adaptate mai multor tipuri de dispozitive. În forma actuală, sistemul este utilizat predominant într-un mediu local. Publicarea controlată a aplicației ar facilita testarea de către un număr mai mare de utilizatori și specialiști, precum și colectarea feedbackului necesar îmbunătățirii experienței de utilizare.

Calitatea componentei RAG poate fi îmbunătățită prin evaluarea controlată a mai multor strategii de retrieval și reranking. Ar putea fi analizate separat efectele interogărilor construite pe variantele de răspuns, ale regăsirii hibride bazate pe baza vectorială și graful de cunoștințe, precum și ale numărului de fragmente transmise modelului. De asemenea, normalizarea automată a abrevierilor medicale ar putea îmbunătăți atât regăsirea contextului, cât și interpretarea grilelor.

Componenta de guardrail poate fi extinsă pentru a diferenția mai precis întrebările educaționale de solicitările medicale reale. Astfel, sistemul ar putea păstra măsurile de siguranță necesare utilizatorilor fără a respinge incorect întrebări formulate prin abrevieri specifice evaluărilor medicale. Evaluatorul ar putea fi, la rândul său, îmbunătățit prin criterii mai fine de verificare a susținerii răspunsului de către context și prin mecanisme mai precise de identificare a erorilor.

O direcție suplimentară este îmbunătățirea transparenței răspunsurilor prin afișarea surselor și a fragmentelor care susțin informațiile generate. Integrarea citărilor direct în interfață ar permite utilizatorului să verifice mai ușor originea informației și ar facilita identificarea situațiilor în care contextul recuperat este incomplet sau nerelevant.

În final, sistemul ar trebui evaluat pe seturi de date mai extinse și validat cu ajutorul specialiștilor medicali. Pentru utilizarea aplicației în afara mediului experimental sunt necesare și mecanisme suplimentare de autentificare, protecție a datelor, jurnalizare securizată și monitorizare continuă a calității răspunsurilor. Optimizarea latenței și a costurilor asociate furnizorilor LLM reprezintă, de asemenea, o condiție importantă pentru dezvoltarea unei soluții accesibile și sustenabile.
