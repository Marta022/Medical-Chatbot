...
Teza este structurată în șase capitole principale:
\begin{itemize}
    \item Capitolul 1, \textit{Fundamente teoretice}, prezintă conceptele necesare înțelegerii lucrării. Sunt introduse rețelele neuronale artificiale, rețelele recurente, arhitectura Transformer, modelele lingvistice de mari dimensiuni, mecanismele de tip Retrieval-Augmented Generation și extinderea acestora prin modele agentice și componente de graf.
    \item Capitolul 2, \textit{Prezentarea tehnologiilor folosite}, descrie instrumentele și platformele utilizate în implementare. Capitolul acoperă limbajul Python, interfața REST și framework-ul Flask, OpenWebUI, Qdrant, tehnicile de segmentare și vectorizare, furnizorii LLM, componenta Kuzu, persistența cu SQLite, containerizarea cu Docker Compose și instrumentele de testare.
    \item Capitolul 3, \textit{Arhitectura proiectului}, detaliază structura sistemului implementat. Sunt prezentate fluxul general al aplicației, orchestratorul, motorul de raționament, modulul de protecție, componenta RAG, integrarea grafului de cunoștințe, evaluatorul, interfața API și mecanismul de persistare a interacțiunilor.
    \item Capitolul 4, \textit{Rezultate Obținute}, prezintă setul de date folosit, metricile de evaluare și experimentele realizate. Sunt analizate efectele strategiilor de retrieval, ale formatului de răspuns, ale prompturilor adaptive și ale mecanismelor de reluare, precum și comparația între modelele LLM evaluate.
    \item Capitolul 5, \textit{Concluzii}, sintetizează rezultatele obținute, evidențiază limitele soluției implementate și propune direcții viitoare de dezvoltare, precum îmbunătățirea regăsirii contextului, extinderea corpusului medical și rafinarea mecanismelor de evaluare.
    \item Capitolul 6, \textit{Fragmente relevante de cod}, include secțiuni reprezentative din implementare. Acestea ilustrează orchestrarea fluxului principal, motorul de raționament, rutarea către furnizorii LLM, endpoint-ul compatibil OpenAI, persistența interacțiunilor, retrieval-ul, ingestia în Qdrant, construirea grafului, regulile de protecție, evaluatorul și benchmark-ul pentru întrebări de tip grilă.
\end{itemize}
...
În mod ideal, antrenarea unui model ar presupune minimizarea erorii de predicție pe întreaga distribuție reală a datelor. În practică, însă, această distribuție nu este cunoscută, iar modelul are acces doar la un set finit de exemple de antrenare. Din acest motiv, optimizarea modelului se realizează prin minimizarea erorii medii calculate pe setul de date disponibil, proces cunoscut sub denumirea de minimizare a riscului empiric \cite{goodfellow2016}.
...
calitativ superior față de modelele generative tradiționale \cite{wang2024survey}. O comparație amănunțită este sintetizată în tabelul de mai jos:
...
euclidiană să fie egală cu 1, transformând toți vectorii în puncte pe o 
...
\section{Construirea blocului de context pentru prompt}

Fragmentul următor arată modul în care liniile de context recuperate sunt transformate într-un bloc textual introdus în promptul final. Funcția păstrează un antet explicit și formatează fiecare fragment recuperat ca element separat, astfel încât modelul să primească informația regăsită într-o formă structurată.

\phantomsection
\label{anexa:prompt-context-builder}
\lstinputlisting[language=Python]{codes/prompt_context_builder.py}
...
\section{Segmentarea semantică a documentelor}

Fragmentul următor prezintă strategia de segmentare semantică utilizată pentru împărțirea textelor în fragmente coerente. Implementarea pregătește textul prin grupare structurală, apoi folosește componenta \texttt{SemanticSplitterNodeParser} din LlamaIndex pentru a obține noduri apropiate semantic; funcția \texttt{chunk\_text} selectează strategia configurată pentru procesul de ingestie.

\phantomsection
\label{anexa:semantic-chunking}
\lstinputlisting[language=Python]{codes/semantic_chunking.py}
...
Importarea datelor în graf extrage entități și relații din fragmentele procesate, creează legăturile cu sursele originale și salvează în Kuzu reprezentarea structurată utilizată de regăsirea hibridă.
