...
Această compatibilitate bidirecțională este esențială pentru integrarea cu OpenWebUI, un proiect open-source independent care oferă o interfață grafică pentru sisteme conversaționale compatibile cu API-ul OpenAI. OpenWebUI nu reprezintă un frontend dezvoltat în cadrul acestei lucrări, ci un serviciu extern utilizat ca strat de prezentare. Prin expunerea endpoint-ului \texttt{/v1/chat/completions} conform specificației OpenAI, backend-ul chatbot-ului poate fi conectat la OpenWebUI fără modificări suplimentare, acesta din urmă tratând serviciul ca pe orice alt furnizor compatibil cu protocolul OpenAI.
...
Stratul de rutare este implementat în modulul \texttt{llm/llm\_router.py} și expune o interfață unificată pentru generarea completărilor de text, indiferent de furnizorul selectat. Selecția furnizorului activ este configurabilă prin variabile de mediu, cu un furnizor principal definit și o ierarhie de alternative activate în caz de eșec.
...
Un element central al arhitecturii multi-provider este integrarea strânsă dintre stratul de rutare și evaluatorul de răspuns. Atunci când evaluatorul detectează un răspuns de calitate insuficientă — prin criterii precum lipsa fundamentării pe contextul recuperat, prezența afirmațiilor contradictorii sau nerespectarea instrucțiunilor de siguranță — modulul de \textit{reasoning} poate relua generarea cu un prompt ajustat sau poate comuta pe un furnizor alternativ din ierarhia configurată.
...
\begin{itemize}
    ...
    \item orchestratorul agentic pentru controlul fluxului (guardrails, delegare către reasoning, returnarea răspunsului validat);
    \item modulul de \textit{reasoning} pentru decizie RAG on/off, construcție de prompt, retry și selecția furnizorului LLM;
